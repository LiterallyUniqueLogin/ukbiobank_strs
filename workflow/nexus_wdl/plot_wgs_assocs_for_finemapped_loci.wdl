...
import "../gwas_wdl/prep_samples_and_phenotype.wdl"
...
workflow plot_wgs_assocs_for_finemapped_loci {
  ...
  scatter (phenotype_idx in range(length(phenotype_names.n))) {
    ...
    call prep_samples_and_phenotypes_workflow.prep_samples_and_phenotypes { input :
      ...
      sc_phenotype = "~{sc_data_dir}/~{phenotypes_names.ID[phenotype_idx]}.tsv"
  ...
  output {
    Array[File] pngs = flatten(locus_plot.png)
    Array[File] svgs = flatten(locus_plot.svg)
  }
}
