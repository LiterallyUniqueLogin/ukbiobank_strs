...
workflow gwas {
  ...
  call prep_samples_and_phenotypes_workflow.prep_samples_and_phenotypes { input :
    ...
    withdrawn_sample_list = withdrawn_samples_list,
    kinship = kinship,
  ...
  scatter (association_region in str_association_regions.out_tsv) {
    ...
    call gwas_tasks.regional_my_str_gwas { input :
      ...
      shared_covars = prep_samples_and_phenotypes.shared_covars,
      untransformed_phenotype = prep_samples_and_phenotypes.pheno_data[0],
      transformed_phenotype = prep_samples_and_phenotypes.transformed_trait_values[0],
      all_samples_list = all_samples_list,
      is_binary = is_binary,
...
#  }
  ...
  call gwas_tasks.prep_plink_input { input :
    script_dir = script_dir,
    shared_covars = prep_samples_and_phenotypes.shared_covars,
    shared_covar_names = prep_samples_and_phenotypes.covar_names,
    transformed_phenotype = prep_samples_and_phenotypes.transformed_trait_values[0],
    pheno_covar_names = prep_samples_and_phenotypes.pheno_covar_names[0],
    is_binary = is_binary,
  ...
  scatter (ethnicity_enumeration in range(length(pheno_data_) -1)) {
    scatter (pair in zip(range(22), get_strs_in_finemapping_regions.strs_in_finemapping_regions)) {
      ...
      call gwas_tasks.regional_my_str_gwas as ethnic_regional_my_str_gwas { input :
        ...
        untransformed_phenotype = prep_samples_and_phenotypes.pheno_data[ethnicity_idx],
        transformed_phenotype = prep_samples_and_phenotypes.transformed_trait_values[ethnicity_idx],
        all_samples_list = all_samples_list,
        is_binary = is_binary,
        binary_type = "linear", # won't be used if not binary
        phenotype_name = phenotype_name,
  ...
  output {
    ...
    Array[File] all_sample_lists = prep_samples_and_phenotypes.all_sample_lists # unqced, and this doesn't take into account the subpop
    Array[File] sample_lists = prep_samples_and_phenotypes.sample_lists # unrelated, qced and takes into account the subpop if specified

    File? subpop_sample_list_input = subpop_sample_list

    File shared_covars = prep_samples_and_phenotypes.shared_covars
    File shared_covar_names = prep_samples_and_phenotypes.shared_covar_names

    # sample lists subset to those with the specified phenotype
    Array[File] all_samples_for_phenotype = prep_samples_and_phenotypes.all_samples_for_phenoypte # qced and takes into account subpop if specified, but not unrelated
    Array[File] samples_for_phenotype = prep_samples_and_phenotypes.samples_for_phenotype # unrelated, qced and takes into account subpop if specified

    Array[File] pheno_data = prep_samples_and_phenotypes.pheno_data # raw
    Array[File] transformed_trait_values = prep_samples_and_phenotypes.transformed_trait_values
    Array[File] pheno_covar_names = prep_samples_and_phenotypes.pheno_covar_names
    Array[File] pheno_readme = prep_samples_and_phenotypes.pheno_readme
