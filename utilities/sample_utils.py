...
def _samples_array_with_indicator(sample_fname):
    ...
    samples_idx = ~np.isnan(samples_merge[:, 1])

    hardcall_samples_merge = utils.merge_arrays(hardcall_snp_samples, samples_indicator)
    assert np.all(~np.isnan(hardcall_samples_merge[:, 1]) == samples_idx)
    assert np.all(hardcall_samples_merge[samples_idx, 0] == samples_merge[samples_idx, 0])
