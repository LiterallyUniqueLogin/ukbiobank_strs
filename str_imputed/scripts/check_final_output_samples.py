...
def check_samples(run_name, chrom, printing=True):
    ...
    with gzip.open(f"{UKB}/str_imputed/runs/{run_name}/vcfs/chr{chrom}.vcf.gz",
                   'rt') as chrom_vcf:
        ...
        if samples != chrom_samples:
            if not printing:
                return False
            print(f"Samples in chromosome {chrom} not the same as original"
                  f"callset samples!",
                  file=sys.stderr)
    ...
    if not printing:
        return True
    print(f"Success! Samples are correct for the final output "
          f"chromosome {chrom} file")
