...
#this method returns True if the chromosome is done with imputation (no jobs were
#launched) and False if jobs were successfully launched
#and returns a string error message if there was a problem
def do_impute(run_name, pfile_dir, sample_file, chr, readme, command_line=True, ignore_jobs=set()):
	def error(msg):
		if command_line:
	...
	if numSamples < 50000:
		error(f"Error: found too few samples ({numSamples}), something went wrong during pfile loading")
	...
	for file_name in glob.glob(f"{ukb}/str_imputed/runs/{run_name}/batches/output/*.e*"):
		with open(file_name) as file:
			...
			if error and current_chrom:
				...
				if len(matches) != 1:
					error("Found more than 1 INPUT1 in an error file, confused")
	...
	for job in jobsToRun:
		...
		sp.run(f"sed -e 's/%RUN_NAME%/{run_name}/g' -e 's/%CHROM%/{chr}/g' -e 's/%SAMPLE%/{job[0]}/g' {ukb}/str_imputed/impute.pbs > {tmpdir}/impute_{run_name}.pbs",
			shell = True, stdout = sp.PIPE, stderr = sp.PIPE)
		if command_line:
			print(f"chr{chr}: Launching job {job}")
		sp.run(f'qsub -v "INPUT1={job[0]},INPUT2={job[1]},INPUT3={chr},INPUT4={pfile_dir},INPUT5={sample_file}" {tmpdir}/impute_{run_name}.pbs',
			 shell = True, stdout = sp.PIPE, stderr = sp.PIPE)
	return False
...
if __name__ == "__main__":
	...
	parser.add_argument("sample_file", help="the .sample file with the list of samples, see $UKB/microarray/*.sample for an example")
	...
	readme = args.readme
	do_impute(run_name, pfile_dir, sample_file, chr, readme)
