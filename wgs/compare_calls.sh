...
zcat "$WGS_STRS" | grep -P '^##' > temp.merged.hg19.vcf
printf "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t" >> temp.merged.hg19.vcf
#also make samples match - ID to ID_ID
zcat "$WGS_STRS" | grep -P '^#CHROM' | cut -f 10- | sed -e 's/\([^\t]\+\)/\1_\1/g' >> temp.merged.hg19.vcf
zcat "$WGS_STRS" | grep -Pv '^#' | cut -f 1 > merged1.tab
zcat "$WGS_STRS" | grep -Pv '^#' | cut -f 3- > merged3on.tab
...
bcftools annotate -h <( echo "##command=HipSTR" ) -o imputed_strs.reheadered.vcf.gz -O z "$IMPUTED_STRS"
